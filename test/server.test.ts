...
tap.test('POST /users/signup', async (t: any) => {
    const response = await server.post('/users/signup').send(mockUser)
    t.equal(response.status, 200)
    t.end()
})

tap.test('POST /users/signup with missing email', async (t: any) => {
...
tap.test('POST /users/login', async (t: any) => {
...
tap.test('POST /users/login with wrong password', async (t: any) => {
    const response = await server.post('/users/login').send({
        email: mockUser.email,
...
tap.test('GET /users/preferences', async (t: any) => {
...
tap.test('GET /users/preferences without token', async (t: any) => {
    const response = await server.get('/users/preferences')
    t.equal(response.status, 401)
    t.end()
})

tap.test('PUT /users/preferences', async (t: any) => {
    const response = await server
...
tap.test('Check PUT /users/preferences', async (t: any) => {
    const response = await server
        .get('/users/preferences')
        .set('Authorization', `Bearer ${token}`)
    t.equal(response.status, 200)
...
tap.test('GET /news without token', async (t: any) => {
